defmodule PersonalSite.Plausible do
  ...
  defp send_event(name, path, user_agent, ip_address, domain, method) do
    ...
    Logger.info("Plausible tracking attempt: enabled=#{enabled}, domain=#{domain}")

    if enabled do
      api_key = Keyword.get(config, :api_key, "")
      Logger.info("Plausible API key present: #{api_key != ""}")
      do_send_event(name, path, user_agent, ip_address, domain, api_key, method)
    else
      Logger.info("Plausible tracking disabled")
    end
  rescue
    error ->
  ...
  defp do_send_event(name, path, user_agent, ip_address, domain, api_key, method) do
    if api_key == "" do
      ...
      Logger.info("Sending Plausible event: #{inspect(event_data)}")

      case Tesla.post(client(), "/api/event", event_data, headers: headers) do
        {:ok, %{status: status}} when status in 200..299 ->
          Logger.info("Plausible event sent successfully: #{path} [#{method}] -> #{status}")
